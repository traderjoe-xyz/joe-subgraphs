...
export function getTokenAmount0(event: Swap): BigInt {
  ...
  const decimalValue = BigInt.fromI32(getDecimals(Address.from(token0)).value as i32)
...
export function getTokenAmount1(event: Swap): BigInt {
  ...
  const decimalValue = BigInt.fromI32(getDecimals(Address.from(token1)).value as i32)
